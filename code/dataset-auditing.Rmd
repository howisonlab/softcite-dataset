...
Get positive coding results on the PMC article set.
...
positive_results <- data.world::query(positive_query, softcite_ds) %>%
  as_tibble() %>% 
...
positive_results %>% group_by(article) %>% 
...
Get PMC articles with negative coding results.
...
no_selection_results <- data.world::query(no_selection_query,
                                           dataset = softcite_ds) %>%
...
no_selection_results %>% group_by(article) %>% 
  summarise(n_coder = n_distinct(coder)) %>% 
  group_by(n_coder) %>% tally()

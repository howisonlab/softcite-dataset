...
author: "James Howison and Fan Du"
date: "3/29/2018"
...
#       format = "csv", extra=list(format="csv"))$results %>% 
  ...
  mutate_at(vars(article, reference, from_in_text_selection),
            list(~ str_extract(.,"[#/]([^#/]+)$"))) %>%
  mutate_at(vars(article, reference, from_in_text_selection), list(~ str_sub(.,2))) %>% 
  filter(!str_detect(article,"^a")) %>% 
...
#      format = "csv", extra=list(format="csv"))$results %>% 
  ...
  mutate_at(vars(selection, code),
            list(~ str_extract(.,"[#/]([^#/]+)$"))) %>%
  mutate_at(vars(selection, code), list(~ str_sub(.,2))) %>% 
  filter(!str_detect(selection, "^a")) %>% 

...
library(tidyverse)
library(SPARQL) # loads saved config see quickstart vignette
library(here)
...
assignments <- SPARQL(url = softcite_ds, query = paste0(prefixes, articles), 
       format = "csv", extra=list(format="csv"))$results %>% 
  as.tibble %>% 
  write_csv(here::here("data/csv_dataset/softcite_assignments.csv"))
```

...
```
**1. The frequency of PMC and econ article sets:**

```{r csv_articles}
csv_articles %>%
  filter(!is.na(no_selections_found)) %>% 
  filter(article_set %in% c("pmc_article", "econ_article")) %>%
  distinct(article, article_set) %>% 
  # 4,971 PMC/econ articles annotated
  tabyl(article_set) %>% 
  adorn_totals("row") %>%
...
csv_articles %>%
  ...
  filter(article_set %in% c("pmc_article", "econ_article")) %>%
  distinct(article, article_set) %>% 
  ggplot(aes(x=article_set, fill=article_set)) + 
  geom_bar(stat = "count") + 
...
**4. Number for mentions in articles:**

```{r mention_in_article}
csv_mentions %>%
  group_by(article) %>% 
...
**5. Among 25,746 annotations in total, we have produced software-related annotations (software_name, software_was_used, version_number, version_date, creator, url).**

```{r}
csv_codes %>% 
  filter(was_code_present == T) %>% 
  distinct() %>% 
  # 25,746 annotations made
  filter(code %in% c("software_name", "software_was_used", "version_number", "version_date", "creator", "url")) %>% 
  tabyl(code) %>% 
  adorn_totals("row") %>% 
  adorn_pct_formatting() %>% 
  knitr::kable() %>% kable_styling()
...
csv_codes %>% 
  ...
  geom_bar(stat = "count") +
  labs(title = "Number of annotations in each field", x="Field label") +

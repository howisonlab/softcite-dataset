...
software_wikipedia <- query_wikidata('SELECT 
  ?item ?itemLabel 
WHERE {
  ...
  as.tibble() %>% 
  mutate(norm_code_label = itemLabel %>% str_to_lower())

# software_wikipedia %>% 
#   filter(norm_code_label %>% str_detect("clustal"))
```


```{r libraries.io, eval=F}

libraries.io <- here::here("Libraries-io","Libraries.io-open-data-1.2.0", "projects-1.2.0-2018-03-12.csv") %>% 
  read_csv %>% 
  janitor::clean_names()

libraries_io_seeds <- libraries.io %>% 
  select(name) %>% 
  distinct() %>% 
  mutate(norm_code_label = name %>% str_to_lower()) %>% 
  write_csv(here::here("Libraries-io","libraries_io_names_only.csv"))

glimpse(libraries_io_seeds)


libraries_io_seeds %>% pull(norm_code_label) %>% length %>% scales::comma()
```
...
# all_seeds <- software_wikipedia %>% 
#   ungroup() %>% 
#   select(norm_code_label) %>% 
#   distinct()  

all_seeds <- read_csv(here::here("Libraries-io","libraries_io_names_only.csv")) %>% 
  select(norm_code_label) %>% 
  # split all components on either / a colon or a period.
  separate_rows(norm_code_label, sep = "[/:.]") %>% 
  distinct()
  
# tribble(
#   ~norm_code_label,
#   "com.github.jeluard",
#   "split/me/up:god.damnit"
# ) %>% 
#   separate_rows(norm_code_label, sep = "[/:.]")
...
experiment <- crossing(seed_percent = seq(0.05, 1, by = 0.05)) %>%
  ...
  mutate(
    num_seeds_used = (floor((all_software_found %>% nrow) * seed_percent)) * 100,
    curr_seeds = map(num_seeds_used, sample_n, tbl = all_seeds))

# experiment %>% slice(1) %>% pull(curr_seeds) %>% pluck(1) %>% nrow
# 
# summary(experiment)
```
...
experiment_with_seed_starts <- experiment %>% 
  ...
  mutate(num_seeds_found = map_int(seed_starts, count_seeds))
...
```{r}
# collapse across replications
summarized_results <- results_of_experiment %>% 
  mutate(expand_window = parse_factor(expand_window, levels=NULL)) %>% 
  group_by(expand_window, num_seeds_used, replication) %>% 
  summarize(
    mean_seeds_found = mean(num_seeds_found),
    mean_proportion = mean(reached_proportion))

summarized_results <- results_of_experiment %>% 
  mutate(pages_read = (expand_window * 2) + 1) %>% 
  # convert expand_window to pages_read
  group_by(pages_read, num_seeds_used) %>%
  # collapse across num_seeds_used (was seed_percent)
  summarize(mean_proportion = mean(reached_proportion),
            mean_seeds_found = mean(num_seeds_found),
            mean_reached = mean(reached_in_window)) %>% 
  ungroup()

# want to have mean_seeds_found be mean_reached for pages_read = 0.
num_seeds_found <- summarized_results %>%
  select(num_seeds_used, mean_seeds_found) %>% 
  distinct() %>%  # because num_seeds_used doesn't depend on pages_read.
  rename(mean_reached = mean_seeds_found) %>% # reached just through text search
...
summarized_results_with_seed_counts <- summarized_results %>% 
  select(-mean_seeds_found) %>% 
  bind_rows(num_seeds_found)
...
summarized_results_with_seed_counts %>% 
  ggplot(aes(x = num_seeds_used, y = mean_proportion, group = pages_read, color = pages_read)) +
...
```{r}
summarized_results_with_seed_counts %>% 
  ggplot(aes(x = pages_read, y = mean_proportion, group = num_seeds_used, color = num_seeds_used)) +
  # scale_color_brewer(type="div", palette = "RdPu") + 
  geom_point() +
  geom_line() +
  scale_x_log10() +

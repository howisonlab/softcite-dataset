...
# runs once for each article and coder
does_match <- function(df) {
  art <- df[[1, "article"]]
  coder <- df[[1, "coder"]]
  others <- found %>% filter(article = art, coder != coder)
  
  my_sel_as_range <- 
}

get_ranges <- function(df) {
  return(df %>% select(start, width) %>% pmap(IRanges))
}

# run for each article and coder
ddply(found, .(article, coder), does_match)
```

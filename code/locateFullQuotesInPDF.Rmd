...
# get 7,026 selections
top_codes <- as_tibble(top_codes) %>% 
  filter(str_detect(article, "PMC"))
# get 5,782 selections only from PMC articles
...
no_selection_articles <- data.world::query(no_selection_query, 
  ...
  mutate(article = str_extract(article, "[#/]([^#/]+)$"),
         ...
         unmatched = 0) %>% 
  collect()
# 3,732 coding without selections. 3,432 distinct coded article without selections, some were multi-coded.
```
...
top_codes %>% summarize(num_articles = n_distinct(article),
                        ...
                        num_selections = n_distinct(selection)) %>% View()
# 1,354 articles coded; 37 coders; 5779 selections
...
path = "C:/Users/D/Documents/Academics/2-Projects/HowisonLab/softcite-pdf-files/docs/pdf-files/pmc_oa_files"
...
(Fan @Jan 29, 2020: looks like this section extracts text from pdf. Do we need it now?)

```{r, eval=F}
# library(extractr)
# Fan @Jan 29, 2020: package abandoned and archived
# library(pdftools)

# # locate PDFs
# folder <-  "/Users/howison/Documents/UTexas/Projects/SloanSoftCite/softcite-dataset/docs/pdf-files/pmc_oa_files/"
# folder <-  "C:/Users/D/Documents/Academics/2-Projects/HowisonLab/softcite-pdf-files/docs/pdf-files/pmc_oa_files"
...
# my_extract <- function(p) {
#   pdf_t <- pdf_text(p)
#   no_linebreaks <- str_replace_all(pdf_t, "[\\r\\n\\s]", "")
#   return(no_linebreaks)
# }
...
# do the selection matching for all the selections that an article has
# this function wraps the match_selection()
...
# match a manually extracted full quote from the input with a single whole article string and calculate some matching statistics at the article level
match_selection <- function(df, pdf_as_xstring) {
  ...
  if (length(results) == 0) {
      ...
      # mismatch_allow <- 10
      #(Fan @Jan 29, 2020: Why picking this threshold value?)
      results <- matchPattern(norm_full_quote, pdf_as_xstring,
                          with.indels = T, max.mismatch = mismatch_allow)
      #(Fan @Jan 29, 2020: only the "best local matches" are returned by matchPattern. i.e., with the shortest edit distance)
  }
  
  #(Fan @Jan 29, 2020: pdf_as_xstring = BString(pdf_as_string). Using the XString object class in BioString)
  if (length(results) == 0) {
...
(found <- top_codes %>% 
  ...
  group_by(article) %>% 
  do(match_in_article(.)))
  found %>% # adds the num_found, start_val, end_val
  filter(num_found == 1) %>% 
  unnest(cols = c(start_val,end_val)) %>% # unlists the start_val and end_val columns # Fan @Jan 29, 2020: unfold the group results for each observation in the group
  ungroup() %>% 
  select(-norm_full_quote, -num_found, -found, -spans_pages) %>% 
  collect() %>% View()
# Fan @Feb 5, 2020: matched the full quote in each selection with the pdf with their offsets
...
(matching <- found %>% 
  ...
  filter(start_val.x <= end_val.y, 
    ...
    # Fan @Feb 5, 2020: cannot compare between 2 lists
  # used as a check

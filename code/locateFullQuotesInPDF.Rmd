...
---
Only working on all the PMC articles.
...
no_selection_articles <- data.world::query(no_selection_query,
  ...
  collect() %>% 
  filter(str_detect(article, "PMC"))
# 1,363 coding without selections. 1,197 distinct coded article without selections, some were multi-coded.
```

SPARQL queries return everything as URLs, but we want just the localPart at the end.

Regex means :Find the last # or / from the end of the string, then get everything.  Thus

https://www.ncbi.nlm.nih.gov/pmc/articles/PMC1538888
...
# 1,354 articles coded; 37 coders; 5,779 selections

top_codes %>% group_by(article, coder) %>%
  summarize(num_selections = n_distinct(selection)) %>%
  arrange(desc(num_selections)) %>%
  filter(article == "PMC1636350")
# coded article with the most selections
...
#
# simple_agree_counts_by_article <- read_csv("simple_agree_counts_by_article.csv")

...
```{r, echo=F, results='hide', message=FALSE, warning=FALSE}
qry_string = "SELECT DISTINCT ?article ?coder ?selection ?quote ?page ?software ?ref
WHERE {
...
As an example below is a screenshot showing the results of querying "yt-project". In the background the system has downloaded the page and parsed it for a citation request. In the case of YT the citation request links to a bibtex file which was then downloaded and parsed to provide the citation.

The second screenshot below shows an example of a user seeking a citation for an R package. The system downloads the URL ttps://cran.r-project.org/web/packages/stringr and identifies a link to a repository (here github). We then seek a file in the repository that makes a citation request, checking README and CITATION / CITATION.TXT. This exploits the R communities adoiption of the CITATION file (which is also parsed when the R module is built providing output to the R citation() command).

The provenance chain is illustrated in the API output below (actually for the CERMINE tool above, CiteAs was provided with the url `https://github.com/CeON/CERMINE`), this shows the locations and techniques that CiteAs attempted in finding a citation. In this case the system identified DOIs located in the README file; the DOIs relate to both an article and to a zenodo archived version of the software.
 ...
 "provenance": [
    {
      ...
      "source_type": "https://github.com/CeON/CERMINE", 
      "success": true
    }, 
    {
      "location": "https://github.com/CeON/CERMINE", 
      "provenance_step_type": "StringProvenanceStep", 
      "source_type": "GitHub repository", 
      "success": true
    }, 
    {
      "location": "https://github.com/CeON/CERMINE/raw/master/citation.rst", 
      "provenance_step_type": "LinkProvenanceStep", 
      "source_type": "GitHub file CITATION", 
      "success": false
    }, 
    {
      "location": "https://github.com/CeON/CERMINE/raw/master/README.md", 
      "provenance_step_type": "LinkProvenanceStep", 
      "source_type": "GitHub file README", 
      "success": true
    }, 
    {
      "location": "https://github.com/CeON/CERMINE/raw/master/README.md", 
      "provenance_step_type": "LinkProvenanceStep", 
      "source_type": "DOI in GitHub README file", 
      "success": true
    }, 
    {
      "location": "https://github.com/CeON/CERMINE", 
      "provenance_step_type": "MetadataProvenanceStep", 
      "source_type": "GitHub", 
      "success": true
    }
  ] 
...
The backend for CiteAs consults what will become the database linking tools to associated citations. Thus far the database is composed from the techniques that Citeas uses to locate citation requests. Over time we will expand this in two ways. First we will run our citation lcoation techniques pre-emptively using catalogues of scientific software (e.g., libraries.io, NSF and NIH listings, and field specific listings such as the ASCL and SBGrid). Second, we will add tool-citation links from the literature including initially those identified through our manual coding, and eventually those identified as our machine learning improves.  As links are added to the database they will become available to CiteAs and our two other tools (CiteSuggest and Software ImpactStory).

## Our collaboration is going well.

...
# set up experiment.
experiment <- crossing(expand_window = 0:1, seed_percent = seq(0.05, 0.10, by = 0.05)) %>%
  crossing(replication = 1:2) %>% 
...
full_experiment_results <- all_software_found %>% 
  ...
  # mutate(seed_mentions = future_map2(curr_seeds, article_label_pages, inner_join, by = "norm_code_label", .progress = T)) %>% 
  ...
  # mutate(reachable_mentions = future_pmap_int(list(seed_mentions_pages, 
  #                                           article_mentions_pages, 
  #                                           expand_window),
  #                                      count_mentions_in_window, .progress=T),

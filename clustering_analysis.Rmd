...
Using these long tables.  For each seed, find starts then divide distribution of page_diff into expand_windows
...
find_reached <- function(curr_long_result, curr_seeds) {
  # seed_starts <- curr_long_result %>% 
    # filter(norm_code_label %in% curr_seeds$curr_seeds)
  ...
  counts <- hist(seed_starts$page_diff, c(0, expand_window, max(seed_starts$page_diff)), plot = FALSE, include.lowest = T) %>% 
                  pluck("counts") %>% 
                  cumsum() %>% # things in lower windows are in higher windows. 
                  head(-1) # drop max   
...
results_of_experiment <- experiment_with_data %>% 
  mutate(total_mentions = map_int(long_result, nrow)) %>% 
...
```{r}
results_of_experiment <- results_of_experiment %>%
  mutate(reached_proportion = if_else(total_mentions == 0, 1, reached_in_window / total_mentions))
# 
